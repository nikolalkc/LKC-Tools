--[[
 ...
 Provides:
  ...
  [Main] ReaOpen - Relative paths setup.lua
  Lokasenna_GUI\Class - Button.dat
  Lokasenna_GUI\Class - Label.dat
  Lokasenna_GUI\Class - Options.dat
  Lokasenna_GUI\Class - Window.dat
  Lokasenna_GUI\Core.dat
 About:
  ...
  It is now possible to add up to four local/network paths to search for projects, based on the Relative path substring.

  - This fixes the issue of opening REAPER projects that were moved locally/were local copies of the original REAPER project
  - This enables the opening of REAPER projects from the depot that were submitted using different local paths

]] --[[
 * Changelog:
  * v1.84beta (2025-02-25)
	+ Added support to define relative paths and four different roots, solves P4 workspace issues
  * v1.83 (2023-05-25)
...
function root1()
    -- Msg(GUI.elms.Label1.caption)
    CHOSEN_ROOT = GUI.elms.Label1.caption
    if GUI.Val("DontAskAgain") == true then
        reaper.SetExtState("ReaOpen","chosen_root",CHOSEN_ROOT,false)
    end
    GUI.quit = true
    RegularReaOpen()
...
function root2()
    -- Msg(GUI.elms.Label2.caption)
    CHOSEN_ROOT = GUI.elms.Label2.caption
    if GUI.Val("DontAskAgain") == true then
        reaper.SetExtState("ReaOpen","chosen_root",CHOSEN_ROOT,false)
    end
    GUI.quit = true
...
function root3()
    -- Msg(GUI.elms.Label3.caption)
    CHOSEN_ROOT = GUI.elms.Label3.caption
    if GUI.Val("DontAskAgain") == true then
        reaper.SetExtState("ReaOpen","chosen_root",CHOSEN_ROOT,false)
    end
    GUI.quit = true
...
function root4()
    -- Msg(GUI.elms.Label4.caption)
    CHOSEN_ROOT = GUI.elms.Label4.caption
    if GUI.Val("DontAskAgain") == true then
        reaper.SetExtState("ReaOpen","chosen_root",CHOSEN_ROOT,false)
    end
    GUI.quit = true
...
function changePaths() Msg("CHANGE PATH") end


function HandleRelativePaths()
    -- RELATIVE_PATHS_CSV = string.gsub(RELATIVE_PATHS_CSV,",","\n\n")
    loadfile(script_path .. [[Lokasenna_GUI]] .. separator .. [[Core.dat]])()
    loadfile(script_path .. [[Lokasenna_GUI]] .. separator .. [[Class - Button.dat]])()
    loadfile(script_path .. [[Lokasenna_GUI]] .. separator .. [[Class - Options.dat]])()
    loadfile(script_path .. [[Lokasenna_GUI]] .. separator .. [[Class - Label.dat]])()
    loadfile(script_path .. [[Lokasenna_GUI]] .. separator .. [[Class - Window.dat]])()

    GUI.name = "ReaOpen - Choose Your Root"
    GUI.x, GUI.y, GUI.w, GUI.h = 0, 0, 800, 300
    ...
    GUI.New("DontAskAgain", "Checklist", {
        z = 11,
        x = 32,
        y = 160,
        w = 260,
        h = 60,
        caption = "DontAskAgain",
        optarray = { "Dont ask again until next REAPER restart" },
        dir = "v",
        pad = 4,
        font_a = 2,
        font_b = 3,
    ...
    for k, v in pairs(PARSED_RELATIVE_PATH_SETTINGS) do
        if v ~= "" then
            if k == 2 then GUI.elms.Label1.caption = v end
            if k == 3 then GUI.elms.Label2.caption = v end
            if k == 4 then GUI.elms.Label3.caption = v end
            if k == 5 then GUI.elms.Label4.caption = v end
        end
    end
...
function open_or_select_tab()
    ...
    --PARSE RELATIVE PATH IF NEEDED
    if reaper.HasExtState("ReaOpen", "chosen_root") then
        local root = reaper.GetExtState("ReaOpen","chosen_root")
        local relative_part = string.match(n1,PARSED_RELATIVE_PATH_SETTINGS[1])
        n1 = root .. relative_part
        -- Msg("ROOT STORED, OPENING:" .. n1)
    else
        if CHOSEN_ROOT ~= nil then
            local relative_part = string.match(n1,PARSED_RELATIVE_PATH_SETTINGS[1])
            n1 = CHOSEN_ROOT .. relative_part
            -- Msg("ROOT CHOSEN, OPENING:" .. n1)
        end
    end
...
function RegularReaOpen()
    ...
    if m ~= nil then
        ...
        if test == 1 then
            ...
            test = string.find(filetxt, '"')
            if test == 1 then filetxt = string.sub(filetxt, 2, testlength - 1) end
        end
        ...
        if m ~= nil then
            ...
            if m ~= nil then filenametemp = string.sub(filenametemp, 1, n) end
                ...
                while not bext_found and fo < file_size do
                    ...
                    if chunk_header ~= "bext" then
                        ...
                        bext_OriginatorReference =
                            string.sub(chunk_data_buf, 256 + 32 + 1, 256 + 32 + 32)
                        bext_OriginationDate = string.sub(chunk_data_buf, 256 + 32 + 32 + 1,
                                                          256 + 32 + 32 + 10)
                        bext_OriginationTime = string.sub(chunk_data_buf, 256 + 32 + 32 + 10 + 1,
                                                          256 + 32 + 32 + 10 + 8) -- left these "open" to show the obvious structure
                        bext_TimeRefLow_buf = string.sub(chunk_data_buf, 256 + 32 + 32 + 10 + 8 + 1,
                                                         256 + 32 + 32 + 10 + 8 + 4) -- SMPTE codes and LUFS data follow these
                        bext_TimeRefHigh_buf = string.sub(chunk_data_buf,
                                                          256 + 32 + 32 + 10 + 8 + 4 + 1,
                                                          256 + 32 + 32 + 10 + 8 + 4 + 4) -- see EBU Tech 3285 v2 etc for more details.
                        bext_VersionNum_buf = string.sub(chunk_data_buf,
                                                         256 + 32 + 32 + 10 + 8 + 4 + 4 + 1,
                                                         256 + 32 + 32 + 10 + 8 + 4 + 4 + 2) --
            ...
            while (i < x) do
                ...
                if take ~= nil then
                    ...
                    retval, stringNeedBig = reaper.GetSetMediaItemTakeInfo_String(take, "P_NAME",
                                                                                  "", false)
                ...
                if its_audio_or_empty_item > 0 then
                    ...
                    -- Msg("")
                    if stringNeedBig == filetxt or stringNeedBig == filetext_with_monkey or XXX[1] ==
                        filetxt or XXX[1] == filetext_with_monkey then
                        -- Msg("FOUND")
            ...
            if weallgood == 0 then
                ...
                while (i < x) do
                    ...
                    if take ~= nil then
                        its_audio_or_empty_item = 1
                        retval, stringNeedBig = reaper.GetSetMediaItemTakeInfo_String(take,
                                                                                      "P_NAME", "",
                                                                                      false)
                        stringNeedBig = string.gsub(stringNeedBig, "\n", "") -- remove newlines
                        stringNeedBig = string.gsub(stringNeedBig, "\r", "") -- remove newlines
                    ...
                    if its_audio_or_empty_item > 0 then
                        ...
                        -- Msg("XXX[1]:"..tostring(XXX[1]))
                        -- Msg("")
                        if stringNeedBig == filetxt or stringNeedBig == filetext_with_monkey or
                            XXX[1] == filetxt or XXX[1] == filetext_with_monkey then
                            -- Msg("FOUND SIMILAR")
                            pos = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
                            reaper.Main_OnCommand(40289, 0) -- unselect all items
                            reaper.SetMediaItemSelected(item, true)
                            ...
                            reaper.ShowMessageBox(
                                "Timecode Offset doesn't match the file selected, but a clip in this session has the same filename, so perhaps this is the correct one...",
                                "Possible Error", 0)
                            break
                        end
                    end
            ...
            if weallgood == 0 then
                ...
                if BBB == false then
                    reaper.ShowMessageBox("Couldn't Find That Filename in the Session...Sorry...",
                                          "ERROR!!!!!!!", 0)
                else
...
-- CHECK RELATIVE STUFF
RELATIVE_PATH_ENABLED = reaper.HasExtState("ReaOpen", "relative_paths")
if RELATIVE_PATH_ENABLED then
    RELATIVE_PATHS_CSV = reaper.GetExtState("ReaOpen", "relative_paths")
    PARSED_RELATIVE_PATH_SETTINGS = ParseCSVLine(RELATIVE_PATHS_CSV, ",")
    if RELATIVE_PATHS_CSV ~= "" then
        has_state = reaper.HasExtState("ReaOpen", "chosen_root")
        if has_state then
            CHOSEN_ROOT = reaper.GetExtState("ReaOpen","chosen_root")
            CHOSEN_ROOT = tonumber(CHOSEN_ROOT)
            RegularReaOpen()
        else
            HandleRelativePaths()
        end
