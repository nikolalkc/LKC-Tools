...
function DoMutate()
    ...
    local states,values = LoadExState(3)
    MUTA.Mutate(MUTA.ConvertChecklistArrayToDict(states),MUTA.ConvertSlidersArrayToDict(values))
    
    reaper.Undo_EndBlock("LKC - Variator - Mutate using formula 3", -1) -- End of the undo block. Leave it at the bottom of your main function.
