...
function DoMutate()
    ...
    local states,values = LoadExState(2)
    MUTA.Mutate(MUTA.ConvertChecklistArrayToDict(states),MUTA.ConvertSlidersArrayToDict(values))
    
    reaper.Undo_EndBlock("LKC - Variator - Mutate using formula 2", -1) -- End of the undo block. Leave it at the bottom of your main function.
